...
const AddPricingSection = ({ token }: PropsTypes) => {
  ...
  const { mutate: updateNumber, isLoading } = useMutation({
    mutationFn: (credentials: any) =>
      customFetch.put(`/api/v2/onboarding/company/send_otp`, credentials, {
        headers: {
  ...
  const { mutate: updateOTP } = useMutation({
    mutationFn: (credentials: any) =>
      customFetch.put(`//api/v2/onboarding/company/verify_otp`, credentials, {
        headers: {
          'Content-Type': `application/json`,
          Authorization: `Bearer ${token}`,
        },
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [`userAuthData`] });
      toast.success(`Phone Number bas been verif`);
    },
    onError: (error: any) => {
      toast.error(error.response.data.message);
    },
                              ...
                              <CustomButton
                                ...
                                onClick={() => {
                                  if (values.phone_number) {
                                    ...
                                    handleVerifyOTP(values.phone_number);
