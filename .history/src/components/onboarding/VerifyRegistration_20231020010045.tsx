...
const VerifyRegistration = ({ token }: PropsTypes) => {
  ...
  const getValidationSchema = (isRegistered: any) => {
    return isRegistered === 'yes' ? CompanySchema : {};
  };
  ...
  validationSchema={getValidationSchema(initialIsRegisteredValue)}
  onSubmit={(values) => {
...
>
Save to grepper
In this approach, you would need to know the initial value of isRegistered when deciding the schema. If the value changes during the form's life, you'll need to reset the form or find another way to re-evaluate the validation schema.
