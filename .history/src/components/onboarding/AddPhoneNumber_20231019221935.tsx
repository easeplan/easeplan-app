...
const AddPricingSection = ({ token }: PropsTypes) => {
  ...
  <Formik
    initialValues={{
      phoneNumber: '',
      verificationToken: '',
      isTokenVisible: 'hidden',
    }}
    validationSchema={YourValidationSchema} // Replace with your actual schema
    onSubmit={(values) => {
    ...
    {({ values, setFieldValue }) => (
      <Form>
        <Box sx={{ flexGrow: 1, width: `100%` }}>
          ...
          <Box style={{ visibility: values.isTokenVisible }}>
          ...
          <Box mt={2}>
            <Button
              type="button"
              onClick={() => {
                setFieldValue('isTokenVisible', 'visible');
                setShowPhoneNumber(false); // Hide the phone number input after verification
              }}
            >
              Verify
            </Button>
          ...
          {/* Next Button */}
          <Box mt={2}>
            <Button type="submit" disabled={values.isTokenVisible === 'hidden'}>
              Next >
            </Button>
          </Box>
