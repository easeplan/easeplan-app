...
const InstagramOnboarding: React.FC = (queryData={queryData}) => {
  const [photos, setPhotos] = useState<Photo[]>([]);
  const [selectedPhotos, setSelectedPhotos] = useState<string[]>([]);

  // Fetching Instagram photos after obtaining token and user ID. This is just a representation.
  const loadPhotos = async () => {
    const data = await fetchInstagramPhotos(`IGQWRPZA1dBV095bTQ1R2dNZAmtRSUsxalhkREpKQ1U4em4taEVDSXBuN19GT09HM3ZAsTDJHaFF4enJoWjQxSG4tQ0lQeDJ3SHN0NzdDODMtTnduNUxmbjlHS09LU3dSZAUtoV0xvdnA3Y2V0RjBvbXZAHS2F5OW9OZAXdnbVZAJYXJCUklzOHMZD`, `23964241119890640`);
    setPhotos(data);
  };
  loadPhotos ()
