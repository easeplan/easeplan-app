...
const AddPricingSection = ({ token }: PropsTypes) => {
  ...
  const { mutate: updateOTP, isLoading } = useMutation({
    mutationFn: (credentials: any) =>
  ...
  const handleVerifyNumber = async (credentials: any) => {
    const resp = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/onboarding/company/send_otp`, credentials, {
      headers: {
        'Content-Type': `application/json`,
        Authorization: `Bearer ${token}`,
      },
                              ...
                              <CustomButton
                                ...
                                onClick={() => {
                                  if (values.destination) {
                                    ...
                                    handleVerifyNumber(values.destination);
