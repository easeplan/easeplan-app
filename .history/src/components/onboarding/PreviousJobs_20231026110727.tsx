...
const PreviousJobs = ({ queryData, token }: any) => {
  ...
  useEffect(() => {
    const refreshToken = async () => {
      ...
      fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/v2/auth/instagram/refresh?userId${queryData.provider._id}&oldToken=${queryData.provider.igAccessToken}`,
      );
    };
  ...
  }, [queryData.provider._id, queryData.provider.igAccessToken]);
