...
const AddPricingSection = ({ token }: PropsTypes) => {
  ...
  const handleVerifyNumber = async (credentials: any) => {
    ...
    updateProfile(data);
    setShowPhoneNumber(false);  // After verifying the number, show OTP code input
  };
  ...
  const handleVerifyOTP = async (credentials: any) => {
    ...
    updateProfile(data);
    handleNextSlide();  // Navigate to the next slide after verifying OTP
  };
  ...
  return (
    <>
      ...
      validationSchema={showPhoneNumber ? NumberSchema : CodeSchema}
      onSubmit={values => {
        showPhoneNumber ? handleVerifyNumber(values) : handleVerifyOTP(values);
      }}
    >
