...
const PreviousJobs = ({ queryData, token }: any) => {
  ...
  const saveSelectedPhotos = async () => {
    ...
    for (const photo of selectedPhotos) {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_UR}/profiles/${queryData?.provider._id}/add-sampleIG`,{
        method: 'PUT',
        headers: { // corrected typo from 'header' to 'headers'
          'Content-Type': "application/json"
        },
        body: JSON.stringify({
          sampleImage: photo.sampleImage,
          title: photo.title,
      ...
      if (!response.ok) {
        toast.error("Image upload for " + photo.title + " failed");
      } else {
        toast.success(`Job Photo ${photo.title} added successfully`);
        //handleNextSlide();
      }
    }
    ...
    <>
      ...
      <Box mt={5} mb={10}>
        ...
        >
          ...
          <div>
            { 
              <Button variant="contained" disabled= {(selectedPhotos.length >=3 || photoCount >= 3) && `true`} onClick={(selectedPhotos.length >=3 || photoCount >= 3) ? saveSelectedPhotos: nex }>
                Proceed
              </Button>
            }
          </div>
